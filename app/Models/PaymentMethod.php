...
class PaymentMethod extends Model
{
    ...
    public function get_payment_method(Request $request): LengthAwarePaginator
    {
        $query = self::query();
        ...
        if ($request->input('sort_order')) {
            $query = self::query()->where('sort_order', $request->input('sort_order'));
        }
    ...
    public function update_payment_method(Request $request, PaymentMethod $payment_method)
    {
        $payment_method->update($this->prepare_data($request));
    }

    public function delete_payment_method(PaymentMethod $payment_method)
    {
        $payment_method->delete();
    }

    public function get_payment_method_for_api(Request $request)
    { {
            return cache()->rememberForever('payment_methods', function () use ($request) {
    ...
    // public function get_payment_method_assoc(){
    //     return self::query()->pluck('name', 'id');
    // }

    public function get_payment_method_assoc($shopId = null)
    {
        $query = self::query();
    ...
    public function get_payment_methods(int $shop_id)
    {
        return self::query()->where('status', self::STATUS_ACTIVE)->where('shop_id', $shop_id)->get();
    }

    public static function getGroupedByShop()
    {
        return self::all()->groupBy('shop_id');
    }
