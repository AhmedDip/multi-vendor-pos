...
<div class="mb-4 col-md-6">
    <div class="custom-form-group">
        {{html()->label('Category slug', 'slug')}}
        <x-required/>
        {{html()->text('slug')->class('form-control form-control-sm '. ($errors->has('slug') ? 'is-invalid' : ''))->placeholder(__('Enter Category slug'))}}
        <x-validation-error :errors="$errors->first('slug')"/>
    </div>
</div>
<div class="mb-4 col-md-6">
    <div class="custom-form-group">
        {{html()->label('Category sort order', 'sort_order')}}
        <x-required/>
        {{html()->text('sort_order')->class('form-control form-control-sm '. ($errors->has('sort_order') ? 'is-invalid' : ''))->placeholder(__('Enter Category sort order'))}}
...
<div class="mb-4 col-md-6">
    <div class="custom-form-group">
        ...
        <x-required/>
        {{html()->select('status',\App\Models\Category::STATUS_LIST)->class('form-select form-select-sm '. ($errors->has('status') ? 'is-invalid' : ''))->placeholder(__('Select Category Status'))}}
...
<div class="mb-4 col-md-6">
    <div class="custom-form-group">
        ...
        <x-required/>
        {{html()->textarea('description')->class('form-select form-select-sm '. ($errors->has('description') ? 'is-invalid' : ''))->placeholder(__('Enter Category description'))}}
    ...
    <div class="col-md-4">
        ...
        @isset($category->photo->photo)
            <div class="text-center">
                <img src="{{get_image($category->photo->photo)}}" alt="image" class="m-auto img-thumbnail">
            </div>
