{{ html()->form('GET', route('warehouse.index'))->id('search_form')->open() }}
...
<div class="mb-4 row justify-content-center align-items-end">
    ...
    <div class="col-md-3">
        ...
        {{html()->text('name', $search['name'] ?? null)->class('form-control form-control-sm')->placeholder(trans('Ex.Outlate-1'))}}
    </div>
    <div class="col-md-3">
        <label for="status">{{ __('Select Status') }}</label>
        {{html()->select('status', \App\Models\Manufacturer::STATUS_LIST, $search['status'] ?? null)->class('form-select form-select-sm')->placeholder(trans('Select Status'))}}
    </div>
